...
def check_all_command_guards(command: str, env_type: str,
    ...
    if is_gateway or is_ask:
        ...
        # Redact secrets in the user-facing copy — the raw `command` is
        # preserved for execution and the allowlist keys off pattern_key, so
        ...
        if miniapp_choice in {"once", "session", "always"}:
            # Match the canonical gateway path: a smart-DENY owner override is
            # one-operation only, even if an older Mini App returns a broader
            # choice. Manual/ESCALATE approvals retain normal persistence.
            if not smart_denied_for_owner:
                for key, _, is_tirith in warnings:
