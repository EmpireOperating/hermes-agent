...
class TestIngestWindows:
    ...
    def test_preserves_missing_z_index_as_none(self):
        from tools.computer_use.cua_backend import _ingest_windows

        out = _ingest_windows([
            {"app_name": "Firefox", "pid": 1, "window_id": 2, "z_index": None}
        ])

        assert out[0]["z_index"] is None
...
def test_capture_vision_survives_missing_z_indices():
    # wlroots exposes null z-order for all windows. The backend must retain
    # driver order and still select the requested app instead of comparing
    # None values while sorting.
    raw = [
        {"app_name": "Terminal", "pid": 1234, "window_id": 1, "z_index": None},
        {"app_name": "Firefox", "pid": 4321, "window_id": 77,
         "is_on_screen": True, "title": "Mozilla Firefox", "z_index": None},
    ]
    backend = _backend_with_windows(raw)
...
def test_focus_app_survives_missing_z_indices():
    # focus_app shares the wlroots window enumeration with capture(), so it
    # must not reintroduce the null-z-index sort crash.
    raw = [
        {"app_name": "Terminal", "pid": 1234, "window_id": 1, "z_index": None},
        {"app_name": "Firefox", "pid": 4321, "window_id": 77,
         "is_on_screen": True, "title": "Mozilla Firefox", "z_index": None},
